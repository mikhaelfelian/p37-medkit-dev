<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>Laporan <small>Data Penjualan</small></h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <div class="row">
            <div class="col-lg-6">
                <?php echo form_open(base_url('laporan/set_lap_penjualan.php'), 'autocomplete="off"') ?>
                    ...
                    <div class="box box-warning">
                        ...
                        <div class="box-body table-responsive">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php if (!empty($penjualan)) { ?>
                                        <?php $no = (!empty($_GET['halaman']) ? $_GET['halaman'] + 1 : 1); ?>
                                        <?php $total = 0; ?>
                                        <?php foreach ($penjualan as $penjualan) { ?>
                                            <?php $total = $total + $penjualan->jml_gtotal; ?>
                                            <?php $tgl = $this->tanggalan->tgl_indo($penjualan->tgl_masuk) ?>
                                            <?php $tgl_tempo = explode('-', $penjualan->tgl_keluar) ?>
                                            <?php $tgl_byr = explode('-', $penjualan->tgl_bayar) ?>
                                            <?php $platform = $this->db->select('tbl_m_platform.platform as metode_bayar, tbl_trans_jual_plat.platform, tbl_trans_jual_plat.keterangan, tbl_m_platform.keterangan as plat_ket')->where('no_nota', $penjualan->no_nota)->join('tbl_m_platform', 'tbl_m_platform.id=tbl_trans_jual_plat.id_platform', 'left')->get('tbl_trans_jual_plat')->row() ?>
                                            <?php $sales = $this->db->where('id_user', $penjualan->id_user)->get('tbl_m_sales')->row() ?>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $tgl . ' ' . $this->tanggalan->wkt_indo($penjualan->tgl_simpan) ?></td>
                                                ...
                                                <td><?php echo (!empty($platform->metode_bayar) ? $platform->metode_bayar : '') . ' ' . ($platform->platform != '-' || !empty($platform->platform) ? ($platform->platform == '-' ? ucwords($platform->plat_ket) : ucwords($platform->platform)) : '') . ' ' . $platform->keterangan ?></td>
