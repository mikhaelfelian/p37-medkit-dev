                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php if (!empty($sql_penj_det)) { ?>
                                                ...
                                                <?php // echo ($_GET['route'] == 'edit' ? form_open('trans_jual_edit_item.php') : '') ?>
                                                <?php // echo ($_GET['route'] == 'edit' AND $_GET/['kd_produk'] == general::enkrip($penj_det->id) ? form_hidden('id', $_GET['id']) : '') ?>
                                                <?php // echo ($_GET['route'] == 'edit' AND $_GET['kd_produk'] == general::enkrip($penj_det->id) ? form_hidden('id_produk', $_GET['kd_produk']) : '') ?>
                                                <?php // echo ($_GET['route'] == 'edit' AND $_GET['kd_produk'] == general::enkrip($penj_det->id) ? form_hidden('route', $_GET['route']) : '') ?>
                                                    <tr>
                                                        <td class="text-center" id="item<?php echo $penj_det->id; ?>">
                                                            <?php echo anchor(base_url('transaksi/cart_jual_upd_hapus.php?id=' . general::enkrip($penj_det->id) . '&no_nota='.$this->input->get('id')), '<i class="fa fa-remove"></i>', 'class="text-danger" onclick="return confirm(\'Hapus [' . $penj_det->produk . '] ?\')"') ?>
                                                        </td>
                                                        <td class="text-center"><?php echo $no++; ?></td>
                                                        <td class="text-left"><?php echo $penj_det->kode ?></td>
                                                        <td class="text-left"><?php echo $penj_det->produk ?></td>
                                                        <?php if($_GET['route'] == 'edit' AND $_GET['kd_produk'] == general::enkrip($penj_det->kode)){ ?>
                                                            <td class="text-right" colspan="6">
                                                                ...
                                                                <div class="input-group date">                                                                    
                                                                    ...
                                                                    <span class="input-group-addon no-border text-bold">&nbsp;</span>
                                                                    <?php echo form_input(array('id' => 'edit_jml', 'name' => 'edit_jml', 'class' => 'form-control', 'value'=>$penj_det->jml, 'style'=>'width: 100px;')) ?>
                                                                    <span class="input-group-addon no-border text-bold">&nbsp;</span>
                                                                    <select name="edit_satuan" class="form-control">
                                                                        <?php foreach ($sql_produk_sat2 as $satuan){ ?>
                                                                            <option value="<?php echo $satuan->satuan ?>" <?php echo ($penj_det->satuan == $satuan->satuan ? 'selected' : '') ?>><?php echo ucwords($satuan->satuan).($satuan->satuan != $sql_satuan->satuanTerkecil ? ' ('.$satuan->jml.' '.$sql_satuan->satuanTerkecil.')' : '') ?></option>
                                                                        <?php } ?>
                                                                    </select>
                                                                    <span class="input-group-addon no-border text-bold">&nbsp;</span>
                                                        ...
                                                        <?php }else{ ?>
                                                            ...
                                                            <td class="text-center"><?php echo ($penj_det->disk1 != 0 ? general::format_angka($penj_det->disk1, 0) : '').($penj_det->disk2 != 0 ? ' + '.general::format_angka($penj_det->disk2, 0) : '').($penj_det->disk3 != 0 ? ' + '.general::format_angka($penj_det->disk3, 0) : '') ?></td>
                                                            <td class="text-right"><?php echo general::format_angka($penj_det->subtotal, 0) ?></td>
                                                            <td class="text-right"><?php echo anchor(base_url('transaksi/trans_jual_edit.php?id='.$this->input->get('id').'&route=edit&kd_produk='.general::enkrip($penj_det->kode).'&aid='.general::enkrip($penj_det->id).'#item'.$penj_det->id),'<i class="fa fa-edit"></i>', '') ?></td>
                                                        <?php } ?>
                                                        ...
                                                        <?php if($_GET['route'] == 'edit' AND $_GET['kd_produk'] == general::enkrip($penj_det->kode)){ ?>
                                                            <td class="text-right">
                                                                <div class="input-group date">
                                                                    <?php echo form_input(array('id' => 'edit_jml', 'name' => 'edit_jml', 'class' => 'form-control', 'value'=>'1', 'style'=>'width: 100px;')) ?>                                                                                                                  <span class="input-group-addon no-border text-bold">&nbsp;</span>
                                                                    <select name="edit_satuan" class="form-control">
                                                                        <option value="<?php echo $satuan2->satuanBesar ?>"><?php echo $satuan2->satuanBesar.' ('.$satuan2->jml.' '.$sql_satuan->satuanTerkecil.')' ?></option>
                                                                        <option value="<?php echo $satuan2->satuanTerkecil ?>"><?php echo $satuan2->satuanTerkecil ?></option>
                                                                        <?php if(!empty($satuan2->satuanGrosir)) { ?>
                                                                            <option value="<?php echo $satuan2->satuanGrosir ?>"><?php echo $satuan2->satuanGrosir ?></option>
                                                                        <?php } ?>
                                                                    </select>
                                                                    <span class="input-group-addon no-border text-bold">&nbsp;</span>
                                                                </div>
                                                        ...
                                                        <?php }else{ ?>
                                                            <td class="text-right"><?php echo $penj_det->jml.' '.$penj_det->satuan.' '.$penj_det->keterangan ?></td>
                                                        <?php } ?>
                                                        <td class="text-right"><?php echo general::format_angka($penj_det->subtotal, 2) ?></td>
                                                        <?php if($_GET['route'] == 'edit' AND $_GET['kd_produk'] == general::enkrip($penj_det->kode)){ ?>
                                                        <td class="text-right"><button type="submit" class="btn btn-flat btn-success"><i class="fa fa-save"></i></button></td>
                                                        <?php }else{ ?>
                                                            <td class="text-right"><?php echo anchor(base_url('transaksi/trans_jual_edit.php?id='.$this->input->get('id').'&route=edit&kd_produk='.general::enkrip($penj_det->kode).'&aid='.general::enkrip($penj_det->id)),'<i class="fa fa-edit"></i>') ?></td>
                                                        <?php } ?>
                                                            -->
                                                    </tr>
                                                <?php } ?>
...
<?php if (!empty($sess_jual)) { ?>
      ...
      $('#kode').autocomplete({
          ...
          select: function (event, ui) {
              ...
              $('#jml').val('1');
              window.location.href = "<?php echo base_url('transaksi/trans_jual_edit.php?id='.$this->input->get('id')) ?>&id_produk="+ui.item.id+"&harga="+ui.item.harga+"#jml"; 
