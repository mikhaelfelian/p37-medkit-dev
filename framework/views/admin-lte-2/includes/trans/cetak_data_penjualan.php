<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>Cetak <small>Transaksi Penjualan</small></h1>
        <ol class="breadcrumb">
                ...
                <div class="box box-warning">
                    ...
                    <div class="box-body">
                        <?php                        
                            ...
                            $filter_tempo = $this->input->get('filter_tgl_tempo');
                            $filter_cust  = $this->input->get('filter_cust');
                            $filter_sales = $this->input->get('filter_sales');
                            $jml          = $this->input->get('jml');
                            
                            $rute         = 'pdf_' . str_replace('data_', '', $this->input->get('route'));
                            $uri          = substr($this->uri->segment(2), 6, -4);
                            $param        = 'transaksi/pdf_'.$uri.'.php?'.(!empty($filter_nota) ? 'filter_nota='.$filter_nota : '').(!empty($filter_tgl) ? '&filter_tgl='.$filter_tgl : '').(!empty($filter_tempo) ? '&filter_tgl_tempo='.$filter_tempo : '').(!empty($filter_cust) ? '&filter_cust='.$filter_cust : '').(!empty($filter_sales) ? '&filter_sales='.$filter_sales : '').(!empty($jml) ? '&jml='.$jml : '');
                        ?>
                        <?php echo anchor(base_url($param) . '&type=D', '<i class="fa fa-download"></i> Unduh', 'target=""') ?>
                        <?php echo nbs(2) ?>
                        <?php echo anchor(base_url($param), '<i class="fa fa-external-link-square"></i> Fullscreen', 'target="_blank"') ?>
                        <?php echo br(2) ?>
                        <div class="embed-responsive embed-responsive-16by9">
                            <iframe class="embed-responsive-item" src="<?php echo base_url($param) ?>">
                            </iframe>
                        </div>
                    </div>
                    <div class="box-footer">
                        <div class="row">
                            <div class="col-lg-6">
                                <button type="button" onclick="window.location.href = '<?php echo base_url('transaksi/'.$uri.'_list.php') ?>'" class="btn btn-primary btn-flat"><i class="fa fa-arrow-left"></i> Kembali</button>
                            </div>
                        </div>
                    </div>
                </div>
