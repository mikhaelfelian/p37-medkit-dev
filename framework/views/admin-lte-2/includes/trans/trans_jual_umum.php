                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <table class="table table-striped">
                                        ...
                                        <tr>
                                            <td><?php echo anchor(base_url('transaksi/trans_jual_umum_draft.php?id='.general::enkrip($draft->id)), $draft->no_nota.(!empty($draft->kode_nota_blk) ? '/'.$draft->kode_nota_blk : ''), '') ?></td>
                                            <td><?php echo anchor(base_url('transaksi/trans_jual_hapus.php?id='.general::enkrip($draft->id).'&route=trans_jual_umum.php?id='.$this->input->get('id')), '<i class="fa fa-remove"></id>', 'class="text-danger" onclick="return confirm(\'Hapus Draft ['.$draft->no_nota.(!empty($draft->kode_nota_blk) ? '/'.$draft->kode_nota_blk : '').']\')"') ?></td>
                                        </tr>
                                        <?php }?>
                                    </table>
                                </div>
                                <?php } ?>
                                <?php echo form_open_multipart(base_url('transaksi/'.(!empty($_GET['id']) ? 'set_nota_jual_umum' : 'set_nota_jual').'.php'), 'autocomplete="off"') ?>
                                <input type="hidden" id="id_customer" name="id_customer">
                                <input type="hidden" id="id_sales" name="id_sales">
                                <input type="hidden" id="no_nota" name="no_nota" value="<?php echo $no_nota ?>">
                                <div class="col-md-<?php echo (!empty($draft) ? '4' : '6') ?>">                                    
                                    <table class="table table-striped">
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle;">                                   
                                                <div id="bag-pelanggan" class="<?php echo (!empty($hasError['pelanggan']) ? 'has-error' : '') ?>">
                                                    <?php echo form_input(array('id' => 'customer', 'name' => 'customer', 'class' => 'form-control text-middle', 'style' => 'vertical-align: middle;', 'value'=>$sql_customer->nama)) ?>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <th style="vertical-align: middle;">:</th>
                                            <td style="vertical-align: middle;">
                                                <?php echo form_input(array('id' => 'cabang', 'name' => 'cabang', 'class' => 'form-control pull-right', 'value'=>$sess_jual['id_app'], 'readonly'=>'TRUE')) ?>
                                            </td>
                                        </tr>
                                        <tr>
                                ...
                                <?php echo form_open_multipart(base_url('transaksi/cart_jual_simpan_umum.php'), 'autocomplete="off"') ?>
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php if (!empty($sql_penj_det)) { ?>
                                                ...
                                                <?php 
                                                    $produk         = $this->db->where('id', $penj_det['options']['id_barang'])->get('tbl_m_produk')->row();
                                                     
                                                    $subtotal       = $penj_det['options']['subtotal']; //$penj_det['qty'] * $penj_det['options']['harga'];
                                                    $jml_tot        = $jml_tot + $penj_det['options']['subtotal']; //($penj_det['price'] * $penj_det['qty']);
                                                    $jml_pot        = $jml_pot + ($penj_det['options']['potongan'] + $penj_det['options']['diskon']);
//                                                     $jml_tot_diskon = $jml_tot_diskon + ($penj_det['options']['diskon'] * $penj_det['qty']);
                                                    $jml_subtotal   = $jml_subtotal + $penj_det['options']['subtotal'];
                                                ?>
                                                    <tr>
                                                        <td class="text-right" style="vertical-align: middle;">
                                                            <?php echo anchor(base_url('transaksi/cart_jual_hapus.php?route='.$this->uri->segment(2).'&id=' . general::enkrip($penj_det['rowid']) . '&no_nota=' . general::enkrip($penj_det['options']['no_nota'])), '<i class="fa fa-remove"></i>', 'class="text-danger" onclick="return confirm(\'Hapus [' . $penj_det['name'] . '] ?\')"') ?>
                                                        </td>
                                                        <td class="text-center" style="vertical-align: middle;"><?php echo $no++; ?></td>
                                                        <td class="text-left" style="width: 150px; vertical-align: middle;"><?php echo $penj_det['options']['kode'] ?></td>
                                                        <td class="text-left" style="width: 450px; vertical-align: middle;"><?php echo $penj_det['name'] ?></td>
                                                        <td class="text-right" style="vertical-align: middle;"><?php echo general::format_angka($penj_det['options']['harga']) ?></td>
                                                        <td class="text-right" style="width: 50px;">
                                                            <?php echo form_open(base_url('transaksi/cart_jual_update_qty.php')) ?>
                                                            <?php echo form_hidden('no_nota', $_GET['id']) ?>
                                                            <?php echo form_hidden('id_cart', $penj_det['rowid']) ?>
                                                            <?php // echo form_hidden('satuan', $penj_det['options']['satuan']) ?>
                                                            <?php echo form_hidden('potongan2', $penj_det['options']['potongan']) ?>
                                                            <?php echo form_input(array('id' => 'qty2', 'name'=>'qty2', 'class'=>'form-control text-center', 'style'=>'width: 50px;', 'value'=>$penj_det['qty'])) ?>
                                                            <?php echo form_close() ?>
                                                        </td>
                                                        <td class="text-left" style="width: 150px; vertical-align: middle;">
                                                            ...
                                                            <!--<select id="satuan" name="satuan" class="form-control">-->
                                                                ...
                                                                <?php // } ?>
                                                            <!--</select>-->
                                                            <?php echo $penj_det['options']['satuan'].' '.$penj_det['options']['satuan_ket'] ?>
                                                        </td>
                                                        <td style="vertical-align: middle;">
                                                            <!--<input type="checkbox" id="harga_ds" name="harga_ds" style="bottom: 190px;" value="1"> Harga DS-->
                                                        </td>
                                                        <td class="text-center" style="vertical-align: middle;"><?php echo general::format_angka($penj_det['options']['disk1'], 0) . (!empty($penj_det['options']['disk2']) ? ' + ' . general::format_angka($penj_det['options']['disk2'], 0) : '') . (!empty($penj_det['options']['disk3']) ? ' + ' . general::format_angka($penj_det['options']['disk3'], 0) : '') ?></td>
                                                        <td class="text-right" style="vertical-align: middle;">
                                                            <?php echo form_open(base_url('transaksi/cart_jual_update_qty.php')) ?>
                                                            <?php echo form_hidden('no_nota', $_GET['id']) ?>
                                                            <?php echo form_hidden('id_cart', $penj_det['rowid']) ?>
                                                            <?php echo form_hidden('qty2', $penj_det['qty']) ?>
                                                            <?php echo form_input(array('id' => '', 'name'=>'potongan2', 'class'=>'form-control text-right', 'style'=>'width: 100px;', 'value'=>$penj_det['options']['potongan'])) ?>
                                                            <?php // echo general::format_angka($penj_det['options']['potongan'], 0) ?>
                                                            <?php echo form_close() ?>
                                                        </td>
                                                        <td class="text-right" style="vertical-align: middle;"><?php echo general::format_angka($penj_det['options']['subtotal'], 0) ?></td>
                                                    </tr>
                                                <?php } ?>
                                                <?php $subtotal = $jml_subtotal - (!empty($sess_pot['jml_retur']) ? $sess_pot['jml_retur'] : '0'); ?>
                                                <?php echo form_open(base_url('transaksi/set_nota_proses_umum.php'), 'id="frm_totalx"') ?>
                                                <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <th colspan="7" class="text-right" style="vertical-align: middle;">Pot. Retur</th>
                                                    <th  class="text-right" style="width: 200px;">
                                                        <?php if(isset($_GET['retur'])){ ?>
                                                        <div class="input-group date">
                                                            <div class="input-group-addon">
                                                                Rp.
                                                            </div>
                                                            <?php echo form_input(array('id' => 'jml_retur', 'name' => 'jml_retur', 'class' => 'form-control pull-right text-right', 'value'=>(isset($_GET['retur']) ? $_GET['retur'] : '0'), 'readonly'=>'TRUE')) ?>
                                                        </div>
                                                        <?php }else{ ?>
                                                                <?php echo anchor(base_url('transaksi/data_retur_jual_list.php?id='.$this->input->get('id').'&route=transaksi/trans_jual_umum.php'), 'Cari Retur') ?>
                                                        <?php } ?>
                                                    </th>
                                                </tr>
                                                <?php if(isset($_GET['retur'])){ ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <th  class="text-right" style="width: 200px;">
                                                        <div class="input-group date">
                                                            ...
                                                            <?php echo form_input(array('id' => 'jml_subtotal', 'name' => 'jml_subtotal', 'class' => 'form-control pull-right text-right', 'readonly' => 'TRUE', 'value' => general::format_angka($subtotal))) ?>
                                                        </div>
                                                    </th>
                                                </tr>
                                            ...
                                            <?php }else{ ?>
                                                <tr>
                                                    <td colspan="12" class="text-center text-bold">Data barang kosong</td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
                            ...
                            <div class="box-footer text-right">
                                <button type="button" class="btn btn-danger btn-flat pull-left" onclick="window.location.href='<?php echo base_url('transaksi/set_nota_batal.php?term=jual_umum&route=transaksi/data_penj_list.php') ?>'"><i class="fa fa-remove"></i> Batal</button>
                                <?php if (!empty($sql_penj_det)) { ?>
                                    <!--<button type="button" class="btn btn-primary btn-flat" onclick="window.location.href='<?php echo base_url('transaksi/cetak_nota_termal.php?id='.$this->input->get('id')) ?>'"><i class="fa fa-print"></i> CETAK</button>-->
                                    <?php echo form_hidden('no_nota', $_GET['id']) ?>
                                    <?php echo nbs() ?>
                                    <button type="button" class="btn btn-warning btn-flat" onclick="window.location.href='<?php echo site_url('page=transaksi&act=set_nota_jual_proses_umum_draft&id='.$this->input->get('id').'&jml_total='.$jml_tot) ?>'"><i class="fa fa-save"></i> [DRAFT]</button>
                                    <?php echo nbs() ?>
...
<?php if (!empty($sess_jual)) { ?>
      ...
      $('#kode').autocomplete({
          source: function (request, response) {
              $.ajax({
                  url: "<?php echo base_url('transaksi/json_barang.php?status=1') ?>",
                  dataType: "json",
                  data: {
                      term: request.term
                  },
