<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>Data Transaksi <small>Retur Penjualan</small></h1>
        <ol class="breadcrumb">
...
<!--                                    <th>Stat Pemb</th>
                            ...
                            <?php echo form_open(base_url('transaksi/set_cari_penj_retur.php')) ?>
                            ...
                            <tbody>
                                ...
                                if (!empty($retur)) {
                                    ...
                                    foreach ($retur as $retur) {
                                        ...
                                        $nota  = $this->db->where('no_nota', $retur->no_nota)->get('tbl_trans_jual')->row();
                                        ?>
                                        <tr>
                                            <td class="text-center"><?php echo $no++ ?>.</td>
                                            <td><?php echo anchor(base_url('transaksi/trans_retur_jual_det.php?id='.general::enkrip($retur->id)), '#'.sprintf("%05s", $retur->no_retur)) ?></td>
                                            <td><?php echo $tgl[1] . '/' . $tgl[2] . '/' . $tgl[0] ?></td>
                                            <td><?php echo anchor(base_url('transaksi/trans_jual_det.php?id='.general::enkrip($retur->no_nota).'&route=transaksi/'.$this->uri->segment(2)), $nota->kode_nota_dpn . $retur->no_nota.'/'.$nota->kode_nota_blk, 'class="text-default"') ?></td>
                                            ...
                                            <td>
                                                <?php if(!empty($sess_ret)){ ?>
                                                    <?php echo anchor(base_url($sess_ret['route'].'?id='.$sess_ret['id'].'&retur='.(float)$retur->jml_retur.'&no_retur='.general::enkrip($retur->id).'&token=1'), 'Input Retur', 'class="label label-primary"') ?>
                                                <?php } ?>
                                                <?php // if($ambil != TRUE){ ?>
