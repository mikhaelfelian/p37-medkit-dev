                ...
                <div class="col-lg-12">
                    <div class="box box-warning">
                        <div class="box-header with-border">
                            <h3 class="box-title"><i class="fa fa-shopping-cart"></i> Form Retur Penbelian</h3>
                        </div>
                        <div class="box-body">
                            <?php echo $this->session->flashdata('transaksi') ?>
                            <div class="row">
                                <?php echo form_open_multipart(base_url('transaksi/'.(!empty($_GET['id']) ? 'set_retur_beli_update' : 'set_retur_beli').'.php'), 'autocomplete="off"') ?>
                                ...
                                <div class="col-md-6">
                                    <table class="table table-striped">
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle;">
                                                <?php if(akses::hakSA() == TRUE OR akses::hakOwner() == TRUE){ ?>
                                                    <?php echo anchor(base_url('transaksi/trans_beli_det.php?id='.$this->input->get('no_nota').'&route=trans_retur_beli.php?no_nota='.$this->input->get('no_nota')), $sql_pemb->no_nota) ?>
                                                <?php }else{ ?>
                                                    <?php echo $sql_pemb->no_nota ?>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <!--<tr>-->
                                            ...
                                            <!--<th style="vertical-align: middle;">:</th>-->
                                            <!--<td style="vertical-align: middle;">-->
                                                <input type="hidden" name="status_ppn" value="1" <?php echo ($sql_pemb->status_ppn == 1 ? 'checked="TRUE"' : '') ?>>
                                            <!--</td>-->
                                        <!--</tr>-->
                                        <!--
                                        <tr>
                                        ...
                                        <tr>
                                            <td style="vertical-align: middle; text-align: right;" colspan="3">
                                                ...
                                                <button type="button" class="btn btn-warning btn-flat" onclick="window.location.href='<?php echo base_url('transaksi/set_retur_beli_batal.php?id='.$this->input->get('id').'&no_nota='.$this->input->get('no_nota')) ?>'">Batal</button>
                                                <?php } ?>
                                ...
                                <?php echo form_close() ?>
                                <?php if (!empty($sess_ret_beli)) { ?>
                                <?php echo form_open_multipart(base_url('transaksi/cart_retur_beli_simpan.php')) ?>
                                            ...
                                            <td style="vertical-align: middle;">
                                                <div id="bag-pelanggan" class="<?php echo (!empty($hasError['pelanggan']) ? 'has-error' : '') ?>">
                                                    <div class="input-group date">
                                                        <?php echo form_input(array('id' => 'jml', 'name' => 'jml', 'class' => 'form-control', 'style' => 'width: 100px;', 'value'=>(isset($_GET['jml']) ? (float)$this->input->get('jml') : '1'))) ?>
                                                            ...
                                                            <span class="input-group-addon no-border text-bold"><?php echo nbs() ?></span>
                                                                <select name="satuan" class="form-control">
                                                                    <?php foreach ($sql_produk_sat as $satuan){ ?>
                                                                       <option value="<?php echo $satuan->satuan ?>" <?php echo ($satuan->satuan == $_GET['satuan'] ? 'selected' : '') ?>><?php echo ucwords($satuan->satuan).($satuan->satuan != $sql_satuan->satuanTerkecil ? ' ('.$satuan->jml.' '.$sql_satuan->satuanTerkecil.')' : '') ?></option>
                                                                    <?php } ?>
                                                                </select>
                                                            <span class="input-group-addon no-border text-bold"><?php echo nbs() ?></span>
                                                        <?php } ?>
                                        ...
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle;">
                                                <div class="input-group date">
                                                    ...
                                                    <?php echo form_input(array('id' => 'harga', 'name' => 'harga', 'class' => 'form-control pull-right', 'value'=>(isset($_GET['harga']) ? $this->input->get('harga') : $hrg_satuan))) ?>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                    ...
                    <div class="col-lg-12">
                        <div class="box box-warning" id="data_pelanggan">
                                                ...
                                                <?php $tot_penj = $tot_penj + $penj_det->subtotal; ?>
                                                    <tr>
                                                        <td class="text-center">
                                                            <?php echo anchor(base_url('transaksi/cart_retur_beli_hapus.php?id='.$this->input->get('id').'&no_nota='.$this->input->get('no_nota').'&id_barang='.general::enkrip($penj_det->id)), '<i class="fa fa-remove"></i>', 'class="text-danger" onclick="return confirm(\'Hapus [' . $penj_det->produk . '] ?\')"') ?>
                                                        </td>
                            ...
                            <div class="box-footer">
                                <?php if (!empty($sql_ret_det)) { ?>
                                    <?php echo form_open(base_url('transaksi/set_retur_beli_proses.php')) ?>
                                        ...
                                        <tbody>
                                            <?php if (!empty($sql_pemb_det)) { ?>
                                                ...
                                                <?php $sql_prod = $this->db->where('kode', $penj_det->kode)->get('tbl_m_produk')->row(); ?>
                                                    <tr>
                                                        <td class="text-center">
                                                            <?php echo anchor(base_url('transaksi/trans_retur_beli.php?id='.$this->input->get('id').'&no_nota='.$this->input->get('no_nota').'&id_produk='.general::enkrip($sql_prod->id).'&harga='.$penj_det->harga.'&satuan='.$penj_det->satuan.'&jml='.(float)$penj_det->jml), '<i class="fa fa-mail-forward"></i>', 'class="text-danger" onclick="return confirm(\'Retur [' . $penj_det->produk . '] ?\')"') ?>
                                                        </td>
                                                        <td class="text-center"><?php echo $no++; ?><i class="fa fa-"></i></td>
                                                        <td class="text-left"><?php echo $penj_det->kode ?></td>
                                                        <td class="text-left"><?php echo $penj_det->produk ?></td>
                                                        <td class="text-right"><?php echo general::format_angka($penj_det->harga) ?></td>
                                                        <td class="text-right"><?php echo (!empty($penj_det->keterangan) ? (float)$penj_det->jml.' '.$penj_det->satuan.' '.$penj_det->keterangan : (float)$penj_det->jml.' '.$penj_det->satuan) ?></td>
                                                        <td class="text-center"><?php echo general::format_angka($penj_det->disk1) . (!empty($penj_det->disk2) ? ' + ' . general::format_angka($penj_det->disk2) : '') . (!empty($penj_det->disk3) ? ' + ' . general::format_angka($penj_det->disk3) : '') ?></td>
                                                        <td class="text-right"><?php echo general::format_angka($penj_det->potongan, 0) ?></td>
                                                        <td class="text-right"><?php echo general::format_angka($penj_det->subtotal, 0) ?></td>
                                                    </tr>
