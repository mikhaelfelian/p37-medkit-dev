...
//                                            $supplier = $this->db->where('id', $beli->id_supplier)->get('tbl_m_supplier')->row();
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right"><?php echo general::format_angka($beli->jml_subtotal) ?></td>
                                                <td>
                                                    <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE OR akses::hakAdminM() == TRUE) { ?>
                                                        <?php // echo anchor(base_url('transaksi/trans_beli_edit.php?id=' . general::enkrip($beli->id)), '<i class="fa fa-edit"></i> Ubah', 'class="label label-default"') ?>
                                                    <?php } ?>
                                                    <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE) { ?>
                                                        <?php // echo anchor(base_url('transaksi/trans_beli_hapus.php?id=' . general::enkrip($beli->id)), '<i class="fa fa-remove"></i> Hapus', 'class="label label-danger" onclick="return confirm(\'Hapus [' . $beli->kode_nota_dpn . $beli->no_nota . (!empty($beli->kode_nota_blk) ? '/' . $beli->kode_nota_blk : '') . '] ?\')"') ?>
                                                    <?php } ?>
                                                    <?php // echo anchor(base_url('transaksi/cetak_nota_beli.php?id=' . general::enkrip($beli->id)), '<i class="fa fa-print"></i> ' . ($beli->cetak == '1' ? 'Cetak Ulang' : 'Cetak'), 'class="label ' . ($beli->cetak == '1' ? 'label-danger' : 'label-primary') . '"') ?>
