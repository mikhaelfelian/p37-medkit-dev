...
header('Content-Disposition: attachment;filename="data_omset_' . date('Ymd') . '.xls"');
    ...
    <tbody>
        ...
        if (!empty($sql_omset)) {
            ...
            foreach ($sql_omset as $penjualan) {
                ...
                $sql_medc   = $this->db->where('id', $penjualan->id)->get('tbl_trans_medcheck')->row();
                ...
                $harga      = ($produk->status_racikan == '1' ? $produk->harga_jual : $penjualan->harga);
                $ass        = $harga * $sql_pnjm->persen; // ($sql_pnjm->persen != 0 ? $ass : $harga)
                $regonya    = ($produk->status == '4' ? ($produk->status_racikan == '1' ? ($sql_pnjm->persen != 0 ? $ass : $harga) : $harga) : $harga); # Jika penjamin asuransi, maka harga obat di tambah sesuai setelan % pada database
                $subtot     = $regonya * $penjualan->jml;
                $disk1      = $regonya - (($penjualan->disk1 / 100) * $regonya);
                $disk2      = $disk1 - (($penjualan->disk2 / 100) * $disk1);
                $disk3      = $disk2 - (($penjualan->disk3 / 100) * $disk2);
                $harga_tot  = (($disk3 - $penjualan->potongan) * $penjualan->jml);
                $harga_disk = (($regonya - $disk3));                
                ...
                <tr>
                    ...
                    <td style="width: 100px; text-align: right;"><?php echo (float)$regonya; ?></td>
                    ...
                    <td style="width: 100px; text-align: right;"><?php echo (float)$remun_tot; ?></td>
                </tr>
...
//                        $disk3_rc   = $disk2_rc - (($resep->disk3 / 100) * $disk2_rc);
                        $harga_totrc= $resep->harga * $resep->jml; // (($disk3_rc - $resep->potongan) * $resep->jml);
                        $diskon_rc  = (($resep->harga - $harga_rc) * $resep->jml);
...
//                    $total      = $sql_omset_row->jml_total;
                    $total_disk = $total_disk + $harga_disk + $tot_disk_rc;
                    $total_pot  = $total_pot + $penjualan->potongan;
                    $total_gt_rc= $total_gt_rc + $tot_gt_rc;
                    $total_grand= $total_gt + $total_gt_rc;
//                    $total_grand= $sql_omset_row->jml_gtotal;
