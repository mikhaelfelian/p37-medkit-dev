        ...
        if (!empty($sql_omset)) {
            ...
            foreach ($sql_omset as $penjualan) {
                $sql_plat       = $this->db->select('tbl_trans_medcheck_plat.id, tbl_trans_medcheck_plat.id_platform, tbl_m_platform.platform AS metode, tbl_trans_medcheck_plat.platform, tbl_trans_medcheck_plat.keterangan, tbl_trans_medcheck_plat.nominal, tbl_m_platform.status_akt')
                                           ->where('tbl_trans_medcheck_plat.id_medcheck', $penjualan->id)
                                           ->join('tbl_m_platform', 'tbl_m_platform.id=tbl_trans_medcheck_plat.id_platform', 'left')
                                           ->get('tbl_trans_medcheck_plat');
                $sql_plat2      = $this->db->where('id', $penjualan->metode)->get('tbl_m_platform')->row();
                $id_rc          = (empty($penjualan->resep) ? $penjualan->id_item : '');
                $sql_item_rc    = $this->db->where('id', $penjualan->id_item)->get('tbl_m_produk')->row();
                $harga      = (!empty($penjualan->resep) ? $sql_item_rc->harga_jual : $penjualan->harga);
                $disk1      = $harga - (($penjualan->disk1 / 100) * $harga);
                $disk2      = $disk1 - (($penjualan->disk2 / 100) * $disk1);
                $disk3      = $disk2 - (($penjualan->disk3 / 100) * $disk2);
                $disk       = ($harga - $disk3) * $penjualan->jml; // ($harga - $disk3 - $penjualan->potongan) * $penjualan->jml;
                $diskon     = ($disk / $harga) * 100;
                
//                if(empty($penjualan->resep)){
                ?>
                <tr>
                    <td style="text-align: left;"><?php echo $this->tanggalan->tgl_indo7($penjualan->tgl_simpan); ?></td>
                    <td style="text-align: left;"><?php echo $penjualan->no_akun ?></td>
                    <td style="text-align: left;"><?php echo $penjualan->pasien ?></td>
...
//                            }
                    ...
                    <td style="text-align: center;">99</td>
                    <td style="text-align: center;"><?php echo $penjualan->kode_item ?></td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: left;"><?php echo (!empty($penjualan->satuan) ? strtolower(ucfirst($penjualan->satuan)) : 'Pcs') ?></td>
                    <td style="text-align: center;"><?php echo (float) ($penjualan->jml > 0 ? $penjualan->jml : '') ?></td>
                    <td style="text-align: right;"><?php echo (float) (!empty($penjualan->resep) ? $sql_item_rc->harga_jual : $penjualan->harga) ?></td>
                    <td style="text-align: right;"></td>
                    <td style="text-align: right;"><?php echo round($disk + $penjualan->potongan, 2) ?><?php // echo (float)$penjualan->potongan ?></td>
                    <td style="text-align: left;"><?php echo $penjualan->item ?></td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;">99</td>
                    <td style="text-align: center;">N/A</td>
                    <td style="text-align: center;">99</td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;"><?php echo ($penjualan->metode == '1' ? 'TRUE' : 'FALSE'); ?></td>
                    ...
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;"><?php echo $this->tanggalan->tgl_indo7($penjualan->tgl_masuk) ?></td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;"></td>
                    <td style="text-align: center;"><?php echo $this->tanggalan->tgl_indo7($penjualan->tgl_masuk) ?></td>
                    <td style="text-align: center;"></td>
...
//                            }
                        ...
                        <td style="text-align: center;"></td>
                        <td style="text-align: left;"><?php echo (!empty($resep->satuan) ? strtolower(ucfirst($penjualan->satuan)) : 'Pcs') ?></td>
