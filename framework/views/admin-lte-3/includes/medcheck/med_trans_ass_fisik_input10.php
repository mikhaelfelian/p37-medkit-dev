<?php echo form_open(base_url('medcheck/set_medcheck_ass_fisik_hsl10.php'), 'autocomplete="off"') ?>
            ...
            <div class="col-7 col-sm-9">
                <div class="tab-content" id="vert-tabs-tabContent">
                    <div class="tab-pane text-left fade show active" id="tab-a" role="tabpanel" aria-labelledby="vert-tabs-home-tab">
                        <div class="row">
                            <div class="col-md-12"><h5 class="card-title text-bold">C. Pemeriksaan Urogenital</h5></div>
                            <div class="col-md-12">
                                <br/>
                                <?php foreach ($sql_ass_urgntl->result() as $ass_urgntl) { ?>
                                    <?php $sql_ck = $this->db->where('id_medcheck', $sql_medc->id)->where('id_medcheck_ass', general::dekrip($this->input->get('id_ass')))->where('id_item', $ass_urgntl->id)->where('tipe', '10')->get('tbl_trans_medcheck_ass_fisik_hsl')->row(); ?>
                                    <div class="form-group row">
                                        <label for="inputEmail3" class="col-sm-4 col-form-label"><small><?php echo $ass_urgntl->penyakit ?></small></label>
                                        <div class="col-sm-3">
                                            <?php echo form_input(array('id' => '', 'name' => 'jntg_jwb[' . $ass_urgntl->id . ']', 'class' => 'form-control pull-right text-center rounded-0', 'value' => $sql_ck->item_value2, 'placeholder' => 'Isikan Hasil ...')) ?>
                                        </div>
                                        <label for="inputEmail3" class="col-sm-5 col-form-label"><small><?php echo $ass_urgntl->satuan ?></small></label>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-striped">
                ...
                <tbody>
                    <?php foreach ($sql_medc_lab_spr_dt as $ass_urgntl) { ?>
                        <?php $sql_spiro = $this->db->where('id', $ass_urgntl->id_lab_spiro_kat)->get('tbl_m_kategori_spiro')->row(); ?>

                        <tr>
                            <td style="width: 250px;" class="text-left"><?php echo $ass_urgntl->item_name ?></td>
                            <td style="width: 200px;" class="text-center"><?php echo general::format_angka2($ass_urgntl->item_value, 2) ?></td>
                            <td style="width: 150px;" class="text-center"><?php echo general::format_angka2($ass_urgntl->item_value2, 2) ?></td>
                            <td style="width: 150px;" class="text-center"><?php echo general::format_angka2($ass_urgntl->item_value3, 2) ?></td>
                            <td style="width: 150px;" class="text-center"><?php echo general::format_angka2($sql_spiro->jml_lln, 2) ?></td>
                            <td style="width: 150px;" class="text-center">
                                <?php echo anchor(base_url('medcheck/set_medcheck_lab_spr_hps_hsl.php?act=' . $this->input->get('act') . '&id=' . $this->input->get('id') . '&id_lab=' . $this->input->get('id_lab') . '&item_id=' . general::enkrip($ass_urgntl->id) . '&status=' . $this->input->get('status')), '<i class="fas fa-trash"></i> Hapus', 'class="btn btn-danger btn-flat btn-xs" onclick="return confirm(\'Hapus [' . $ass_urgntl->item_name . '] ?\')" style="width: 65px;"') ?>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
