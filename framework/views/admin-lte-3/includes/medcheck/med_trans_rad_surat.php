                    ...
                    <div class="form-group">
                        ...
                        <select id="dokter_krm" name="dokter_kirim" class="form-control select2bs4 <?php echo (!empty($hasError['dokter']) ? ' is-invalid' : '') ?>">
                            ...
                            <?php foreach ($sql_doc as $doctor) { ?>
                                ...
                                <option value="<?php echo $doctor->id_user ?>" <?php echo ($doctor->id_user == $sql_medc->id_dokter ? 'selected' : '') ?>><?php echo (!empty($doctor->nama_dpn) ? $doctor->nama_dpn.' ' : '').$doctor->nama.(!empty($doctor->nama_blk) ? ', '.$doctor->nama_blk : '') ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="inputEmail3" class="">Dokter Radiologi</label>
                        <select id="dokter" name="dokter" class="form-control select2bs4 rounded-0 <?php echo (!empty($hasError['dokter']) ? ' is-invalid' : '') ?>">
                            <option value="">- Dokter -</option>
                            <?php foreach ($sql_doc_rad as $doctor) { ?>
                                <option value="<?php echo $doctor->id_user ?>" <?php echo (!empty($sql_medc_rad_rw->id_dokter) ? ($doctor->id_user == $sql_medc_rad_rw->id_dokter ? 'selected' : '') : (($doctor->id == $this->session->flashdata('dokter') ? 'selected' : ''))) ?>><?php echo (!empty($doctor->nama_dpn) ? $doctor->nama_dpn.' ' : '').$doctor->nama.(!empty($doctor->nama_blk) ? ', '.$doctor->nama_blk : '') ?></option>
                            <?php } ?>
                        </select>
                    </div>
                <?php } ?>
