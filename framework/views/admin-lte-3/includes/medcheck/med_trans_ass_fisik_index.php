...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">ASSESMENT - <?php echo $sql_pasien->nama_pgl; ?> <small><i>(<?php echo $this->tanggalan->usia($sql_pasien->tgl_lahir) ?>)</i></small></h3>
    </div>
    <div class="card-body">
        <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE OR akses::hakOwner2() == TRUE OR akses::hakDokter() == TRUE OR akses::hakPerawat() == TRUE OR akses::hakAnalis() == TRUE) { ?>
            <?php // if ($sql_medc->status < 5) { ?>
                <a href="<?php echo base_url('medcheck/set_medcheck_ass_fisik.php?id=' . general::enkrip($sql_medc->id) . '&status=' . $this->input->get('status')) ?>" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Assesment</a>
            <?php // } ?>
        <?php } ?>
            ...
            <tbody>
                ...
                <?php foreach ($sql_medc_ass->result() as $asses) { ?>
                    <tr>
                        ...
                        <td class="text-left" style="width: 50px;"><?php echo $this->tanggalan->tgl_indo2($asses->tgl_simpan); ?></td>
                        <td class="text-left" style="width: 50px;">
                            <?php echo $asses->no_sample; ?>
                        </td>
                        <td class="text-left" style="width: 100px;"><?php echo $this->ion_auth->user($asses->id_farmasi)->row()->first_name; ?></td>
                        <td class="text-left" style="width: 90px;">
                            <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE OR akses::hakOwner2() == TRUE OR akses::hakFarmasi() == TRUE OR akses::hakDokter() == TRUE OR akses::hakPerawat() == TRUE OR akses::hakAnalis() == TRUE) { ?>
                                <?php echo anchor(base_url('medcheck/tambah.php?act=ass_surat&id=' . general::enkrip($sql_medc->id) . '&id_ass=' . general::enkrip($asses->id) . '&status=' . $this->input->get('status')), 'Sample &raquo;', 'class="btn btn-success btn-flat btn-xs text-bold" style="width: 70px;"') ?>
                                <?php echo anchor(base_url('medcheck/tambah.php?act=ass_fisik&id=' . general::enkrip($sql_medc->id) . '&id_ass=' . general::enkrip($asses->id) . '&status=' . $this->input->get('status')), 'Input &raquo;', 'class="btn btn-success btn-flat btn-xs text-bold" style="width: 70px;"') ?>
