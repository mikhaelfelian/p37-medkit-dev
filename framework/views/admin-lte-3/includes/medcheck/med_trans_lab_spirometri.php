<div class="card">
    <div class="card-header">
        <h3 class="card-title"><?php echo (isset($_GET['judul']) ? $_GET['judul'] : 'INSTALASI LABORATORIUM') ?> - <?php echo $sql_pasien->nama_pgl; ?> <small><i>(<?php echo $this->tanggalan->usia($sql_pasien->tgl_lahir) ?>)</i></small></h3>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-4 col-sm-3">
                <div class="nav flex-column nav-tabs h-100" id="" role="" aria-orientation="vertical">
                    <?php $this->load->view('admin-lte-3/includes/medcheck/med_trans_lab_sidebar') ?>
                </div>
            </div>
                    ...
                    <div class="tab-pane text-left fade show active" id="tab-pemeriksaan" role="tabpanel" aria-labelledby="vert-tabs-home-tab">
                        <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE OR akses::hakOwner2() == TRUE OR akses::hakDokter() == TRUE OR akses::hakPerawat() == TRUE OR akses::hakAnalis() == TRUE) { ?>
                            <?php // if ($sql_medc->status < 5) { ?>
                                <a href="<?php echo base_url('medcheck/set_medcheck_lab_spr.php?id=' . general::enkrip($sql_medc->id) . '&status=' . $this->input->get('status')) ?>" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Permintaan Spiro</a><?php echo br(2) ?>
                            <?php // } ?>
                        <?php } ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left" style="width: 90px;">
                                            <?php if (akses::hakSA() == TRUE OR akses::hakOwner() == TRUE OR akses::hakOwner2() == TRUE OR akses::hakAnalis() == TRUE OR akses::hakDokter() == TRUE OR akses::hakPerawat() == TRUE) { ?>
                                                <?php echo anchor(base_url('medcheck/tambah.php?act=lab_spirometri_surat&id=' . general::enkrip($sql_medc->id) . '&id_lab=' . general::enkrip($lab->id) . '&status=' . $this->input->get('status')), 'Surat &raquo;', 'class="btn btn-success btn-flat btn-xs text-bold" style="width: 70px;"') ?>
                                                <?php if($lab->status == 1){ ?>
                                                    <?php echo anchor(base_url('medcheck/tambah.php?act=lab_spirometri_input&id=' . general::enkrip($sql_medc->id) . '&id_lab=' . general::enkrip($lab->id) . '&status=' . $this->input->get('status')), 'Input &raquo;', 'class="btn btn-warning btn-flat btn-xs text-bold" style="width: 70px;"') ?>
                                                <?php } ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
