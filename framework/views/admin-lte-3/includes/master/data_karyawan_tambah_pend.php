                                    ...
                                    <?php foreach ($sql_kary_pend as $pend) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-left" style="width:70px;">
                                                <?php echo anchor(base_url('master/set_karyawan_hapus_pend.php?id=' . general::enkrip($pend->id) . '&id_karyawan=' . general::enkrip($sql_kary->id) . (!empty($pend->file_name) ? '&file_name=' . $pend->file_name : '')), '<i class="fa fa-trash"></i>', 'class="btn btn-danger btn-flat btn-xs" onclick="return confirm(\'Hapus Berkas [' . $pend->no_dok . '] ?\')"') ?>
                                            </td>
                                        </tr>
                                        <?php $no++ ?>
                                    <?php } ?>
